// Att fixa: strength (att baserat på lvl), sifferkänslighet (fixa så att det inte kan loopa på inmatning)
# include <iostream>
# include <ctime>
using namespace std;
int main()
{
	int hp1, hp2, hpstart1, hpstart2, type1, type2, typetest1=1, typetest2=1, att1, att2, atthelp1, atthelp2, atttest1=1, atttest2=1, grassplus1, fireplus1, waterplus1, grassplus2, fireplus2, waterplus2,lvl1, lvl2, lvltest1=1, lvltest2=1, igen=1, ask=1, turn=0, first=1;
	char ene='y';
	srand(time(NULL));
	while (lvltest1==1)
	{
	...
	while (hp1>0&&igen==1)
	{
		...
		if (ene=='y')
		{
			...
			if (type1==3)
				cout << "Water" << endl;
			cout << "HP: " << hp1 << "/" << hpstart1 << endl << endl;
			while (lvltest2==1)
			{
			...
			if (type2==3)
				cout << "Water" << endl;
			cout << "HP: " << hp2 << "/" << hpstart2 << endl << endl;
			while (hp1>0&&hp2>0)
			{
				if (turn%2==0)
				{
					...
					if (atthelp1==2&&type1==1)
						att1=1+rand()%4+grassplus1;
					if (atthelp1==2&&type1==2)
						att1=1+rand()%4+fireplus1;
					if (atthelp1==2&&type1==3)
						att1=1+rand()%4+waterplus1;
					if(att1<0)
						att1=0;
					hp2-=att1;
				...
				if (turn%2==1)
				{
					...
					if (atthelp2==2&&type2==1)
						att2=1+rand()%4+grassplus2;
					if (atthelp2==2&&type2==2)
						att2=1+rand()%4+fireplus2;
					if (atthelp2==2&&type2==3)
						att2=1+rand()%4+waterplus2;
					if(att2<0)
						att2=0;
					hp1-=att2;
